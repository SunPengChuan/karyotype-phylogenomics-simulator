...
CONFIG = dict(
    ...
    rearrangement_counts=dict(
        inversion_prob=0.8,   # Probability of inversion per branch (0-1)
        translocation_rct=0.4,  # Probability of RCT per branch (0-1)
        fusion_ncf=0.2,       # Reduced to 0.2 (20% chance per branch)
        fusion_eej=0.3,       # Reduced to 0.3 (30% chance per branch)
    ),
    rct_pairing_mode="random",
    eej_end_mode="random",
    
    # Whole Genome Duplication (WGD)
    wgd_probability=0.05,     # Reduced from 0.1
...
class _RCTOutgroupSignificance:
    ...
    def main() -> None:
        ...
        with open(out_path, "w", encoding="utf-8", newline="") as f:
            ...
            w.writerow(["species", "score_obs", "null_mean", "null_sd", "pvalue", "z", "replicates", "seed"])
            for sp in species:
                ...
                w.writerow([sp, obs, f"{null_mean:.6f}", f"{null_sd:.6f}", f"{p:.6g}", f"{z:.6f}", len(null_scores), seed])
...
class EvolutionEngine:
    def __init__(self, config: ConfigDict):
        ...
        self.used_root_child_fusion_pairs: set[tuple[str, str]] = set()
    ...
    def apply_evolution(self, tree: Tree, root_karyo: Karyotype, wgd_targets: set[str] | None = None, event_plan: dict[str, dict[str, Any]] | None = None) -> None:
        ...
        for node in tree.traverse("preorder"):
            ...
            def apply_plan(karyo: Karyotype, node_obj: Any, name_for_log: str, plan_for_node: dict[str, Any]) -> None:
                ...
                if not (node_obj.up and node_obj.up.is_root()):
                    if random.random() < float(plan_for_node.get("translocation_rct", 0.0)):
                        self._apply_rct(karyo, name_for_log)
                if random.random() < float(plan_for_node.get("fusion_ncf_p", 0.0)):
                    ...
                    if is_root_child and self.cfg.get("avoid_duplicate_root_fusions", True):
                        forbidden = self.used_root_child_fusion_pairs
    ...
    def _apply_rct(self, karyo, node):
        ...
        if len(g1) < 4 or len(g2) < 4: return
